...
// Remove invalid Excel sheet characters & limit to 31 chars
function sanitizeSheetName(name) {
    return name.replace(/[:\\/?*\[\]]/g, "").substring(0, 31);
}

// Convert CSV/TXT text into array-of-arrays
...
// Extract TRN_DATE from CSV header row
...
// ======================================================
// UPLOAD CSV / TXT (EOD FILES)
// ======================================================

uploadCSVBtn.addEventListener("click", async () => {
    if (!uploadedWorkbook) return alert("Please upload Excel first.");

    const files = [...multiUploadFiles.files];
    if (!files.length) return alert("Select CSV/TXT files.");
    ...
    alert(`${files.length} CSV/TXT file(s) loaded.`);
});


// ======================================================
// PROCESS EACH EXCEL SHEET
// Adds formulas, description & total columns
// ======================================================
...
function processSheet(sheet) {
    ...
    // Layout:
    // [DATA] | EMPTY | DESCRIPTION | TOTAL | EMPTY | EOD...
...
// ======================================================
// ADD HEADER FORMULAS (ROW 1 – GREEN CELLS)
// ======================================================
...
// ======================================================
// ADD DESCRIPTION & TOTAL COLUMNS (YELLOW)
// ======================================================
...
processDownloadBtn.addEventListener("click", async () => {
    ...
    // 1️⃣ Process all existing sheets
    uploadedWorkbook.eachSheet(sheet => processSheet(sheet));

    // 2️⃣ Append CSV/EOD data AFTER total + empty column
    csvFilesData.forEach(file => {
        const sheetName = sanitizeSheetName(file.trnDate);
        const sheet =
            uploadedWorkbook.getWorksheet(sheetName) ||
            uploadedWorkbook.addWorksheet(sheetName);

        const lastDataCol = getLastUsedDataColumn(sheet);
        const eodStartCol = lastDataCol + 2; // data | empty | desc | total | empty | EOD
