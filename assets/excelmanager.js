...
// ======================================================
// PROCESS & DOWNLOAD FINAL EXCEL (ALL IN ONE BUTTON)
// ======================================================
...
processDownloadBtn.addEventListener("click", async () => {
    ...
    // 3️⃣ Process all existing sheets
    uploadedWorkbook.eachSheet(sheet => processSheet(sheet));

    // 4️⃣ Append CSV/EOD data
    ...
    // 5️⃣ Download final file
    const title = updateExcelTitleInput.value.trim() || "Updated_Excel";
    const bufferOut = await uploadedWorkbook.xlsx.writeBuffer();
